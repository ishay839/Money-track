...
```bash
git clone https://github.com/ishay839/Money-track.git money-track
cd money-track
npm install
npm run setup
```
...
```bash
npm run update:check   # what would change, nothing touched
npm run update         # pull, rebuild, restart
```

`data/` is never touched by an update: your transactions, budgets, bank
credentials and encryption key are not in git and are left exactly as they
are. Schema changes ship as migrations and are applied automatically the next
time the app starts.

If the build fails, the update stops and the version you were already running
keeps running.

**If you have edited the code yourself**, the update refuses to run rather
than overwrite your work, and tells you how to keep it:

```bash
git stash          # set your changes aside
npm run update
git stash pop      # put them back on top
```

## Making it your own

The app is a normal Next.js + SQLite project, so you can change it yourself -
with [Claude Code](https://claude.com/claude-code) or by hand. `CLAUDE.md` in
the repo root gives an AI assistant the context it needs (stack, conventions,
where things live).

```bash
npm run dev            # http://127.0.0.1:3000, hot reload
npm run service:reload # rebuild + restart the always-on app
```

Two things worth knowing before you edit:

- `next start` serves a **prebuilt** bundle, so a source change does nothing
  until you rebuild. That is what `service:reload` is for.
- Database changes belong in `src/server/db/migrations/` as a new numbered
  `.sql` file. They run once, in order, on the next start. A migration runs
  against databases that already hold real data, so write it to be safe on a
  ledger that is already full - and never to renumber or rename what a user
  has built.

To keep your own changes and still take updates, commit them on a branch:

```bash
git checkout -b my-changes
git commit -am "my tweaks"
# later:
