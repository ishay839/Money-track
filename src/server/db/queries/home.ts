...
export function getCashFlow(
  ...
  const income = db
    .prepare(
      // Offsets (income filed under an expense category) already show up as
      // reduced spend; counting them as income too would double their effect.
...
export function getNeedsAttentionCounts(
  ...
  const uncategorized = db
    .prepare(
      `SELECT COUNT(*) as count FROM operating_transactions
  ...
  const items = db
    .prepare(
      `SELECT id, date, description, ABS(charged_amount) as amount,
              review_reason as reason, kind
       FROM operating_transactions
       WHERE workspace_id = ? AND needs_review = 1 AND status = 'completed'
       ORDER BY ABS(charged_amount) DESC, date DESC
       LIMIT 4`
    )
    .all(workspaceId) as HomeNeedsAttention["items"];
  return {
...
export function getBankHealth(workspaceId: number): HomeBankHealthItem[] {
  ...
  return creds.map(({ provider }) => {
    const latest = latestRunStmt.get(workspaceId, provider) as
      ...
      | undefined;
    const providerInfo = BANK_PROVIDERS.find((p) => p.id === getBaseBankProvider(provider));
    const suffix = provider.split(":")[1];
    const providerName = `${providerInfo?.name ?? provider}${suffix ? ` · חשבון ${suffix}` : ""}`;
...
export function getCategorySnapshot(
  ...
  for (const [key, spent] of rolledSpend) {
    ...
    if (!cat) continue;
    const budget = (rolledBudget.get(key) ?? 0) * budgetMultiplier;
    items.push({
      categoryId: key,
      name: cat.name,
      color: cat.color,
