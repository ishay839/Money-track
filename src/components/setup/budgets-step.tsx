    ...
    <div className="mx-auto w-full max-w-[520px] space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-sm leading-relaxed text-muted-foreground">
          הזינו סכום לקטגוריות שתרצו להגביל. השאירו ריק כדי לעקוב בלבד.
        </p>
      </header>
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <span>
          הוגדר יעד ל-{budgeted.length} מתוך {categories.length} קטגוריות
        </span>
        {total > 0 && (
          <span className="font-bold tabular-nums text-foreground">
            ₪ {total.toLocaleString("he-IL")} לחודש
          </span>
        )}
      </div>
      ...
      <div className="flex justify-center pt-1">
        ...
        >
          דילוג והגדרה אוטומטית לפי ההוצאות לאחר הסנכרון הראשון
        </button>
      </div>
    </div>
  );
