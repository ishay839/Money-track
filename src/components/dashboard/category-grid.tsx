...
export function CategoryGrid({
  ...
  // ?category=<id> opens that category's detail straight away, so settings can
  // link here instead of duplicating the whole breakdown.
  const requested = searchParams.get("category");
  useEffect(() => {
    if (!requested) return;
    const id = Number(requested);
    if (Number.isInteger(id)) setSelectedId(id);
    // Drop the parameter so closing the sheet does not reopen it on the next
    // render, and so a refresh does not fight the user.
  ...
  if (activeCategories.length === 0) {
    ...
    return (
      <div className="space-y-5">
        <h2 className="text-xl font-bold tracking-tight">{t("budgetsHeading")}</h2>
        <div className="surface p-10 md:p-14">
          <div className="mx-auto max-w-md text-center">
            ...
            {hasUncategorized ? (
              ...
              <>
                <h3 className="text-xl font-bold tracking-tight">{t("emptyNoSpendTitle")}</h3>
                <p className="mt-2 text-sm text-muted-foreground">
                  {t("emptyNoSpendBody")}
                </p>
              </>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            {filterLabels.map((f) => {
              ...
              return (
                <button
                  ...
                  className={`inline-flex cursor-pointer items-center gap-1.5 rounded-full px-3.5 py-1.5 text-sm font-semibold transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background ${
                    active
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <Select value={sort} onValueChange={(v) => v && setSort(v as Sort)}>
            <SelectTrigger className="h-9 w-[170px] cursor-pointer text-sm transition-colors duration-200 hover:bg-secondary hover:text-foreground">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
