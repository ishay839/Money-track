...
function ModelStatusInner({
  ...
  if (installed) {
    return (
      <div className="flex items-center gap-2 rounded-md border border-primary/30 bg-primary/5 px-3 py-2 text-sm">
        ...
        <span>
          <span className="font-medium">{modelName}</span> מותקן ומוכן לשימוש.
        </span>
      </div>
    );
  }
      ...
      <div className="space-y-3 rounded-xl border border-border bg-card/60 p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-1">
            <div className="text-sm font-medium">Ollama עדיין לא מותקן?</div>
            <p className="text-xs text-muted-foreground">
              Ollama הוא שירות בינה מלאכותית חינמי שפועל במחשב. ההתקנה אורכת
              כדקה.
            </p>
          </div>
          <a
        ...
        <ol className="space-y-1 ps-5 text-xs text-muted-foreground list-decimal marker:text-muted-foreground/70">
          <li>הורידו והפעילו את תוכנת ההתקנה מאתר Ollama.</li>
          <li>הפעילו את Ollama. הוא יופיע באזור ההתראות של המחשב.</li>
          <li>
            חזרו למסך הזה. המערכת תפעיל את Ollama ותאפשר להוריד את המודל.
          </li>
        </ol>
        <p className="text-xs text-destructive">
          לא ניתן להתחבר ל-Ollama בכתובת הזו. אם הוא כבר מותקן, בדקו שוב את
          הכתובת למעלה.
        </p>
      </div>
    );
  ...
  return (
    <div className="space-y-2 rounded-md border border-dashed bg-muted/20 p-3">
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          <div className="text-sm font-medium">המודל אינו מותקן</div>
          <p className="text-xs text-muted-foreground">
            הורדת <span className="font-medium">{modelName}</span> עכשיו
            {modelInfo
              ? ` (כ-${modelInfo.sizeGb} גיגה-בייט, מספר דקות בהתאם לחיבור)`
              : ""}
            . צריך לבצע זאת פעם אחת בלבד.
          </p>
        </div>
...
function formatBytes(bytes: number): string {
  ...
  const units = ["בתים", "קילובייט", "מגה-בייט", "גיגה-בייט"];
...
function formatDuration(seconds: number): string {
  ...
  if (seconds < 3600) {
    ...
    return s === 0 ? `${m} דקות` : `${m} דקות ${s} שניות`;
  }
  const h = Math.floor(seconds / 3600);
  const m = Math.round((seconds % 3600) / 60);
  return m === 0 ? `${h} שעות` : `${h} שעות ${m} דקות`;
}
