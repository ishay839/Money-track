...
export async function GET(request: Request) {
  ...
  for (const parent of categories) {
    ...
    const directSpend = currentMap.get(parent.id);

    const spent =
      (directSpend?.amount ?? 0) + kidRows.reduce((s, r) => s + r.spent, 0);
    const transactionCount = (directSpend?.count ?? 0) + kidRows.reduce(
      (s, r) => s + r.transactionCount,
      0
    );
    const needsReviewCount = (needsReviewMap.get(parent.id) ?? 0) + kidRows.reduce(
      (s, r) => s + r.needsReviewCount,
      0
    );
    const prevTotal = (prevMap.get(parent.id) ?? 0) + kids.reduce(
    ...
    const directTopMerchant = topMerchantMap.get(parent.id);
    if (directTopMerchant) {
      topMerchant = directTopMerchant.merchant;
      topMerchantAmount = directTopMerchant.amount;
    }
    for (const k of kids) {
