...
export async function POST(request: Request) {
  ...
  if (body.provider === "openai" && body.apiKey) {
    const { encrypted, iv, authTag } = encrypt(body.apiKey);
    setSetting("ai_openai_api_key_encrypted", encrypted.toString("hex"));
    setSetting("ai_openai_api_key_iv", iv.toString("hex"));
    setSetting("ai_openai_api_key_auth_tag", authTag.toString("hex"));
  }
