...
export async function GET(
  ...
  if (isParent) {
    if (budgetSource !== "own") {
      const typical =
        (allAuto.find((a) => a.categoryId === categoryId)?.amount ?? 0) +
        children.reduce(
        (s, c) =>
  ...
  const topMerchants = isParent
    ? aggregateTopMerchants(
        targetIds.map((targetId) =>
          getTopMerchantsForCategory(workspaceId, targetId, from, to, 12)
        )
      )
  ...
  return NextResponse.json({
    ...
    vsLastMonth,
    prevSpent,
    prevPeriodLabel: prevMonthStart.toLocaleDateString("he-IL", {
      month: "long",
    }),
    dailySpend,
    topMerchants,
