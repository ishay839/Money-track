...
/**
 * Money is always shown in whole shekels - no agorot anywhere in the UI.
 *
 * Rounding is to the NEAREST shekel rather than always up: rounding up would
 * make every rounded part drift above the rounded total, so a column of
 * category amounts would no longer add up to the header figure. The stored
 * value keeps its full precision either way; this only affects display.
 */
export function formatCurrency(
  ...
  if (currency === "ILS") {
    return `₪${whole.toLocaleString(bcp, {
